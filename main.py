...
def sonaveebSearch():
    ...
    otsing = otsingEntry.get()
    if otsing != '':
        result = requests.get(f'https://sonaveeb.ee/search/unif/dlall/dsall/{otsing}/1')

        rc = result.content
        soup = BeautifulSoup(rc, features='html.parser')
        try:
            tahendus = soup.find('span', class_='homonym-intro d-block').text
            sunonuumid = soup.find('span', class_='homonym-matches').text
            if sunonuumid:
                sisu = f'Tähendus:\n{tahendus}\n \nSünonüümid: {sunonuumid}'
                textBox.insert(INSERT, sisu)
            else:
                sisu = f'Tähendus:\n{tahendus}'
                textBox.insert(INSERT, sisu)
...
ekssButton = Button(root, text='EKSS', width=10, command=ekssSearch, activebackground='green').grid(row=2, column=1)
iesButton = Button(root, text='IES', width=10, command=iesSearch, activebackground='green').grid(row=2, column=2)
osButton = Button(root, text='ÕS', width=10, command=osSearch, activebackground='green' ).grid(row=2, column=3)
susButton = Button(root, text='SÜS', width=10, command=susSearch, activebackground='green').grid(row=2, column=4)
...
# row 4 ->
sonaveebButton = Button(root, text='Sõnaveeb', width=10, command=sonaveebSearch, activebackground='green').grid(row=4, column=1)
